...
import { createBlog, fetchBlogById, updateBlog } from "@/redux/slices/blogSlice";
...
const BlogForm = ({ blogId }: { blogId?: string }) => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchBlogById(blogId))
      ...
      .then((result) => {
        if (active) {
          setFormData({ title: result.blog.title, content: result.blog.content });
        }
      })
      .catch((error) => {
        if (active) {
          toast.error(typeof error === "string" ? error : "Unable to load blog");
        }
      });
        ...
        <div className="rounded-lg bg-white p-4 shadow-md sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {errors.content && (
                <p id="content-error" className="mt-1 text-sm font-medium text-red-700" role="alert">
                  {errors.content}
                </p>
              )}
            </div>
            ...
            <div className="flex gap-4 pt-6 justify-center">
              <button
                type="submit"
                disabled={loading}
                className="min-h-11 cursor-pointer rounded bg-blue-700 px-4 py-2 font-medium text-white transition duration-200 hover:bg-blue-800 disabled:cursor-not-allowed disabled:bg-gray-600"
              >
